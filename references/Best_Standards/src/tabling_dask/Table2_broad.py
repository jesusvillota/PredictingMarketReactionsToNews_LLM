# uv run src/tabling_dask/Table2_broad.py
#---------------------------------------------------------------
...
from src.tabling_dask.common import build_parquet_filters, create_standard_size_bins
...
def build_table(parquet_filters: list, output_dir, ddf=None):
    ...
    logger.info("Starting Table2_broad.build_table() with Dask.")
    required_columns = [
    ...
    with DaskManager() as dask_manager:
        ...
        filtered_dfs = create_standard_size_bins(ddf, size_col='prtSize_agg')
        ...
        logger.info("Computing total counts per size bin for Table2_broad...")
        total_counts = {
            cat: int(filtered_dfs[cat].shape[0].compute()) for cat in categories
        }
        ...
        output_path = out_dir / "Table2_broad.tex"
    ...
    \midrule
    \textbf{Total} & & {all_tot} & {b1_tot} & {b2_tot} & {b3_tot} \\
    \bottomrule
    \end{tabular}
 \end{table}
"""
        ...
        # Fill totals
        placeholders = ["all_tot", "b1_tot", "b2_tot", "b3_tot"]
        safe_template = _escape_latex_template(latex_content, placeholders)
        latex_filled = safe_template.format(
            all_tot=f"{total_counts['all']:,}",
            b1_tot=f"{total_counts['1_10']:,}",
            b2_tot=f"{total_counts['11_200']:,}",
            b3_tot=f"{total_counts['over_200']:,}",
        )
        ...
        logger.info("Table2_broad.build_table() completed successfully.")
