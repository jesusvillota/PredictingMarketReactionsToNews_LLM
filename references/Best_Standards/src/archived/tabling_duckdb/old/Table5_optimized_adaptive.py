# uv run src/tabling/Table5_optimized_adaptive.py
#---------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    with AdaptiveDaskManager() as dask_manager:
        ...
        for cat in categories:
            ...
            category_total = cat_data['count'].sum() if len(cat_data) > 0 else 0
            
            for strategy in strategy_types:
                strategy_data = cat_data[cat_data['strategy_type'] == strategy]
                count = int(strategy_data['count'].iloc[0]) if len(strategy_data) > 0 else 0
    ...
    \begin{tabular}{lcccc}
    \toprule
        \textbf{Strategy Type} & \textbf{All} & \textbf{1--10} & \textbf{11--200} & \textbf{>200} \\
    \midrule
"""
        
        for strategy in strategy_types:
            values = [format_count_pct(table_data[cat][strategy], category_totals[cat]) 
                     for cat in categories]
